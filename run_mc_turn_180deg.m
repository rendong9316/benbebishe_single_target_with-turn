...
% =========================================================================
% 完全遵循 run_simulation_turn_180deg.m 的 Phase 0-9 流水线。
% 架构: 回头弯航迹 + IMM( CV+CT, Pd-IPDA似然, Pi=[.90 .10] )。
...
fprintf('回头弯几何: 正东(90°)入 → 左转180°半圆(1°/s) → 正西(270°)出\n');
fprintf('  转弯率: %.4f rad/s, 转弯段: 180s ≈ 6帧 @30s\n', turn_rate_rad_per_sec);

%% ---- 打印表头 ----
fprintf('╔══════════════════════════════════════════════════════════════╗\n');
fprintf('║   回头弯180°蒙特卡洛仿真  N=%d  IMM:CV+CT(Pd-IPDA,Pi=[.90 .10])  ║\n', N_MC);
fprintf('╠══════════════════════════════════════════════════════════════╣\n');
fprintf('║ Pd=0.6  Pfa=0.001  R1_Kloss=8 R2_Kloss=8  dt=30s  n_frames≈41  ║\n');
fprintf('║ 转弯: 1°/s 左转, 180°, 180s/6帧, 约在帧19-25                    ║\n');
fprintf('╚══════════════════════════════════════════════════════════════╝\n\n');
...
function [wp, turn_angle_deg, omega] = get_turn_info(params)
    % 回头弯180°: 正东(90°)入, 左转, 正西(270°)出
    W1 = [125.5, 33.0];
    % 中间拐点为虚拟（NaN），终点在出弯后
    W3_lon = 125.5;  W3_lat = 33.0;  % 近似（与create_uturn_trajectory一致）
    wp = [W1(1), W1(2); NaN, NaN; W3_lon, W3_lat];
    turn_angle_deg = 180.0;
    omega = +1.0 * pi / 180.0;  % 左转 +1°/s
end
