% refine_config51 — Targeted search around config#51 to fix right_rate_0p7 failure.
% Usage: matlab -batch "addpath(genpath('D:/Desktop/single_target_with-turn')); refine_config51();"
%
% Strategy: Fix gain (main suspect for right_rate_0p7) + try elevated cv_dwell
% Each candidate: 10 scenes x 20 seeds = 200 evals. Scan 2 params x 5 levels = 25 candidates.

addpath(genpath('D:/Desktop/single_target_with-turn'));
refine_config51();

function refine_config51()
    fprintf('============================================================\n');
    fprintf('Refinement Around Config #51 — Fix right_rate_0p7\n');
    fprintf('============================================================\n\n');
    ...
    default_rmse = squeeze(mean(bl.all_pos_rmse(1, :, :), 3)); % [10,1]
    ...
    % --- Factorial scan: gain x cv_dwell ---
    gains = [11.0, 9.0, 7.0, 5.0, 3.5];
    cv_dwells = [2500, 3000, 4000, 5000, 6000];

    fprintf('Scanning %d x %d = %d gain x cv_dwell combos (%d seeds each)...\n', ...
        length(gains), length(cv_dwells), length(gains)*length(cv_dwells), n_seeds);
    ...
    for gi = 1:length(gains)
        for cdi = 1:length(cv_dwells)
            ri = ri + 1;

            c = struct(...
                'imm_cv_dwell_time_sec', cv_dwells(cdi), ...
                'imm_ct_dwell_time_sec', base_ct_dwell, ...
                'imm_ct_fixed_Q_scale', base_ct_q, ...
                'imm_transient_gain_max', gains(gi), ...
            ...
            fprintf('[%d/%d] gain=%.1f cv_dwell=%d... ', ...
                ri, length(gains)*length(cv_dwells), gains(gi), cv_dwells(cdi));
            ...
            results(ri).gain = gains(gi);
            results(ri).cv_dwell = cv_dwells(cdi);
            results(ri).score = info.score;
            results(ri).protected = info.protected;
            results(ri).scenario_rmse = info.rmse_per_scenario;

            if info.protected && info.score < best_score
                best_score = info.score;
                best_protected = true;
                best_cfg = c;
                fprintf('  --> NEW BEST PROTECTED CONFIG!\n');
            end
        end
    end

    % Print top 5 protected + top 5 overall
    fprintf('\n=== TOP 5 PROTECTED CONFIGS ===\n');
    protected_results = results([results.protected]);
    if ~isempty(protected_results)
        [s, o] = sort({protected_results.score});
        for k = 1:min(5, numel(s))
            idx = o(k);
            r = protected_results(idx);
            fprintf('  gain=%.1f cv_dwell=%d: score=%.3f\n', r.gain, r.cv_dwell, r.score);
            for si = 1:10
                delta = (r.scenario_rmse(si) - default_rmse(si)) / default_rmse(si) * 100;
                status = '';
                if delta > 5, status = ' *** FAILS ***'; end
                fprintf('    %-22s: %.3f (%+.1f%%%s)\n', specs{si}{2}, r.scenario_rmse(si), delta, status);
            end
        end
    else
        fprintf('  No protected configs found in this scan.\n');
    end

    fprintf('\n=== TOP 5 OVERALL (including unprotected) ===\n');
    [s_all, o_all] = sort({results.score});
    for k = 1:min(5, numel(s_all))
        idx = o_all(k);
        r = results(idx);
        fprintf('  #%d: gain=%.1f cv_dwell=%d: score=%.3f\n', idx, r.gain, r.cv_dwell, r.score);
    end

    % Save results
    save(fullfile(root, 'refine_config51_results.mat'), ...
        'results', 'best_cfg', 'best_score', 'best_protected', ...
        'specs', 'default_rmse', 'base_ct_dwell', 'base_ct_q');

    fprintf('\nResults saved to %s/refine_config51_results.mat\n', root);

    if best_protected
        fprintf('\n*** FOUND PROTECTED CONFIG! ***\n');
        fprintf('Best config (gain=%.1f, cv_dwell=%d):\n', best_cfg.imm_cv_dwell_time_sec, best_cfg.imm_cv_dwell_time_sec);
        fnames = fieldnames(best_cfg);
        for fi = 1:numel(fnames)
            fprintf('  %-30s = %.1f\n', fnames{fi}, best_cfg.(fnames{fi}));
        end
    else
        fprintf('\nNo protected config found in this scan. Need wider search.\n');
    end
end
