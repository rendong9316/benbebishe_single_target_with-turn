% run_fuzzy_quick_compare.m — Quick fuzzy_only vs 3in1 comparison
% Run with: matlab -batch "cd D:\Desktop\single_target_with-turn\validation; run(run_fuzzy_quick_compare)"
...
fprintf('=== QUICK COMPARISON ===\n');
for si = 1:n_scenarios
    m3 = all_rmse{1}(si);
    mf = all_rmse{2}(si);
    delta = (mf - m3) / m3 * 100;
    fprintf('  %-20s: 3in1=%.3f fuzzy=%.3f delta=%+.1f%%\n', specs{si}{2}, m3, mf, delta);
end
...
if overall_delta > 20
    fprintf('CONCLUSION: fuzzy_only is >20%% worse than 3in1 on quick test.\n');
    fprintf('Recommendation: focus search on 3in1 mode only.\n\n');
elseif overall_delta < -10
    fprintf('CONCLUSION: fuzzy_only is >10%% better than 3in1 on quick test!\n');
    fprintf('Recommendation: dedicate search to fuzzy_only mode.\n\n');
else
    fprintf('CONCLUSION: fuzzy_only is within +/-20%% of 3in1. Worth searching both.\n\n');
end
