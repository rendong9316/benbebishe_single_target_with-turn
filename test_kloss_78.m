% test_kloss_78.m — 实测 K=7/8，与 K=6 对比 (3×100 种子)
clear; close all; clc; addpath(genpath('.'));
fprintf('K_loss=6/7/8 实测对比, N=100 each...\n\n');
...
summary.ukf_R1_med = zeros(1,3); summary.ukf_R1_mean = zeros(1,3);
summary.ukf_R2_med = zeros(1,3);
summary.fus_med = zeros(1,3); summary.fus_mean = zeros(1,3);
summary.mtl_R1 = zeros(1,3); summary.mtl_R2 = zeros(1,3);
summary.brk_R1 = zeros(1,3);
summary.nis_R1 = zeros(1,3);
summary.assoc_R1 = zeros(1,3);
summary.imp_R1 = zeros(1,3);
...
for ci = 1:3
    ...
    bad = zeros(N,1); n_kloss_trig = 0;

    for mc = 1:N
        seed = mc;
        params = simulation_params(); params.random_seed = seed;
        params.radar1_tracker_K_loss = kl; params.tracker_K_loss = kl;
        rng(seed);
        ...
        params.gate_sigma=params.radar1_gate_sigma; params.gate_vr_ms=params.radar1_gate_vr_ms; params.tracker_K_loss=kl;
        u1=ukf_jichu('create',params,params.radar1_lon,params.radar1_lat,params.radar1_tx_lon,params.radar1_tx_lat,params.dt_sec);
        [sR1,ft1]=single_track_runner(dR1,u1,params,nf,tt,t1);
        ...
        % Count K_loss triggers for diagnostic
        % (check if type went to 7=LOST at any point)
        was_lost = false;
        for k=1:nf
            if ~isempty(sR1{k})&&~isempty(sR1{k}.trackList)
                if sR1{k}.trackList{1}.type==7, was_lost=true; end
            end
        end
        if was_lost, n_kloss_trig = n_kloss_trig + 1; end

        % Bad seed
        if r1_rmse(mc)>30 || r2_rmse(mc)>30
            bad(mc)=1;
        elseif r1_rmse(mc)>15 || r2_rmse(mc)>15
            bad(mc)=1;
        end
    ...
    et=toc;
    % Extract clean data (remove NaN)
    vf=fus_rmse(~isnan(fus_rmse)); v1=r1_rmse(~isnan(r1_rmse)); v2=r2_rmse(~isnan(r2_rmse));

    summary.ukf_R1_med(ci)=median(v1); summary.ukf_R1_mean(ci)=mean(v1);
    summary.ukf_R2_med(ci)=median(v2);
    ...
    summary.kloss_trig(ci)=n_kloss_trig;

    fprintf('  K=%d: %.0fs | bad=%.0f%% (%d/100) | R1_med=%.1f R1_mean=%.1f | Fus_med=%.1f Fus_mean=%.1f | Fus>5km=%.0f%% Fus>10km=%.0f%% | K_loss trig=%d\n', ...
        kl, et, summary.bad_rate(ci), summary.n_bad(ci), summary.ukf_R1_med(ci), summary.ukf_R1_mean(ci), ...
        summary.fus_med(ci), summary.fus_mean(ci), summary.fus_pct_gt5(ci), summary.fus_pct_gt10(ci), n_kloss_trig);
end
...
fprintf('%-16s %7.1f %7.1f %7.1f\n', 'UKF R1均值(km)', summary.ukf_R1_mean);
...
fprintf('%-16s %7d %7d %7d\n', 'Kloss触发次数', summary.kloss_trig);
fprintf('%-16s %7.0f %7.0f %7.0f\n', '耗时(s)', summary.elapsed);

% 判断趋势
fprintf('\n趋势分析:\n');
if summary.fus_med(3) < summary.fus_med(2) && summary.fus_med(2) < summary.fus_med(1)
    fprintf('  融合中位: 单调改善 K6→K7→K8\n');
elseif summary.fus_med(3) > summary.fus_med(2)
    fprintf('  融合中位: K8 反超 K7 (开始恶化!) — K=7 是甜点\n');
elseif abs(summary.fus_med(3) - summary.fus_med(2)) < 0.3
    fprintf('  融合中位: K7 和 K8 基本持平 — 边际收益归零\n');
end
if summary.fus_mean(3) > summary.fus_mean(2) + 0.5
    fprintf('  融合均值: K8 明显恶化 (死航迹污染显现!)\n');
end
if summary.kloss_trig(3) < 0.02 * N
    fprintf('  K=8 下 Kloss 几乎从不触发 — 参数已经冗余\n');
end
