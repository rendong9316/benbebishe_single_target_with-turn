% generate_comparison_plots_v3 — 全场景滤波前后对比可视化
% Usage: matlab -batch "addpath(genpath('D:/Desktop/single_target_with-turn')); generate_comparison_plots_v3();"
% 数据策略: RMSE = 2雷达 × 10 seeds平均（与最终验证一致的量级）
%         轨迹图 = radar R1, seed=10001（便于直观对比）

function generate_comparison_plots_v3()
    ...
    for si = 1:n_scenarios
        ...
        for radar_id = 1:2
            for sk = 1:n_seeds_rmse
                ...
                rmse_c = rmse_c + run_one_eval(prepared, params, radar_id, ov, []);
            end
        end
        ...
        % Plot trajectory comparison (single run, R1, seed=10001)
        plot_single_scenario(si, n_scenarios, chinese_name, short_name, ...
...
% ===========================================================================
% run_one_eval — copies from final_validate_config51.m (verified working)
% ===========================================================================
function rmse_km = run_one_eval(inp, params, radar_id, overrides, _unused)
    if radar_id == 1
...
% ===========================================================================
% plot_single_scenario — trajectory visualization (R1, seed=10001 only)
% ===========================================================================
function plot_single_scenario(fig_num, n_total, ch_name, sh_name, ...
        scenario_name, seed_val, turn_rate, cfg51, outdir, ...
        rmse_def, rmse_cfg)
    ...
    tp_def = ukf_imm('create', rp, ...
        p1.radar1_lon,p1.radar1_lat,p1.radar1_tx_lon,p1.radar1_tx_lat,rp.dt_sec);
    [~,~,sn_def] = run_oracle_tracker_sequence(detList_R1, tp_def, p1, truth_input, inp.t1_grid, false);

    % Config#51 track
    rp2 = radar_params(p1, 1);
    fn = fieldnames(cfg51);
    for fi = 1:numel(fn), rp2.(fn{fi}) = cfg51.(fn{fi}); end
    rp2.imm_adapt_mode = '3in1';
    tp_cfg = ukf_imm('create', rp2, ...
        p1.radar1_lon,p1.radar1_lat,p1.radar1_tx_lon,p1.radar1_tx_lat,rp2.dt_sec);
    ...
    plot_truth_and_tracks(truthTrajs, detList_R1, ...
        trk_def(:,1), trk_def(:,2), trk_cfg(:,1), trk_cfg(:,2));
...
% ===========================================================================
% collect_track_xy — extract [lon, lat, error_m, time_s] from snapshots
% ===========================================================================
...
% ===========================================================================
% plot_truth_and_tracks — spatial view
% ===========================================================================
