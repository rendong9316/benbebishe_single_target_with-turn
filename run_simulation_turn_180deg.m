% =========================================================================
% run_simulation_turn_180deg.m — 双基地OTH-SWR单目标回头弯(180°)IMM仿真主程序
% =========================================================================
% 【程序定位】
%   本程序是本仿真系统的回头弯180°场景主入口。与run_simulation_turn.m
%   （渐进拐弯46.7°）不同，本程序专门验证180°回头弯机动场景下IMM
%   （交互多模型）跟踪相对于单模型UKF的性能提升。核心对比：CV-UKF × IMM
%
% 【回头弯航迹设计】
%   航路点: W1(125.5°E,33.0°N) → 正东飞120km → 入弯点
%           → 左转180°半圆(1°/s, R=13.2km, 180s) → 出弯点
%           → 正西飞120km → W3
%   全部位于双雷达威力交汇区内（~127°E,33°N, 距R1/R2约1200-1300km）
%
% 【IMM（交互多模型）】
...
%   Phase 9: 可视化 + 数据保存
%
% 【与 run_simulation_turn.m 的关键区别】
%   1. 航迹：180°回头弯 vs 46.7°渐进拐弯
%   2. 转弯段：180s/6帧 vs 47s/2帧，充分检验IMM切换能力
% =========================================================================
...
%% ==================== Phase 0: 场景初始化（回头弯目标） ====================
fprintf('========== Phase 0: 场景初始化 (回头弯180°) ==========\n');

params = simulation_params();
rng(params.random_seed);

% ---- 回头弯航迹生成 ----
% aircraft_trajectory_create('uturn') 内部：
%   1. 定义W1起点，正东飞120km到入弯点
%   2. 以1°/s转弯率左转180°，生成半圆段点迹
%   3. 出弯后正西飞120km到W3
%   4. 构建兼容 traj 结构体
[traj, turn_waypoints] = aircraft_trajectory_create('uturn', params);
true_track = aircraft_trajectory_interpolate('generate', traj);
fprintf('真实航迹 (回头弯180°): %d 点, 总时长 %.0f s, 速度 %.0f m/s\n', ...
    size(true_track,1), traj.duration_sec, params.aircraft_speed_ms);

% ---- 计算转弯方向和角速率（用于CT模型） ----
% U-turn: 正东(90°)入，左转180°，正西(270°)出
turn_rate_rad_per_sec = +1.0 * pi / 180.0;  % 左转 +1°/s
fprintf('  转弯率: +1.0°/s (左转半圆), CT模型ω=%.4f rad/s\n', ...
...
outf = fullfile('results', sprintf('simulation_turn180_%s.mat', datestr(now, 'yyyymmdd_HHMMSS')));
save(outf, 'sysPara', 'calibResult', 'truthTraj', 'R1', 'R2', 'params', ...
    ...
    'turn_waypoints', 'turn_rate_rad_per_sec');
fprintf('数据已保存: %s\n', outf);
fprintf('\nDone.\n');
