% =========================================================================
% scan_Q_scale.m — Q_scale 参数扫描主入口（内联 MC 逻辑）
% =========================================================================
% 功能：系统扫描 Q_scale 参数，评估对拐弯场景跟踪性能的影响
% 用法：在 MATLAB 中运行 scan_Q_scale
% =========================================================================
addpath(genpath('.'));
...
for qi = 1:n_q
    ...
    % ==== 场景 1: gradual_turn ====
    fprintf('\n  >>> 场景 1: gradual_turn <<<\n');
    run_mc_gradual_turn(N_MC, SEED_BASE, UKF_NAMES, N_UKF, FUSION_METHODS, N_FUS, q_val);
...
function run_mc_gradual_turn(N_MC, SEED_BASE, UKF_NAMES, N_UKF, FUSION_METHODS, N_FUS, q_val)
    % 预分配
    for u = 1:N_UKF
...
function run_mc_180deg_uturn(N_MC, SEED_BASE, UKF_NAMES, N_UKF, FUSION_METHODS, N_FUS, q_val)
    for u = 1:N_UKF
        s(u).name = UKF_NAMES{u};
        s(u).rmse_ukf_R1 = nan(N_MC, 1);
        s(u).rmse_ukf_R2 = nan(N_MC, 1);
