% =========================================================================
% run_mc_turn_180deg_compare.m — 回头弯180度场景四体制 UKF 对比蒙特卡洛仿真
% =========================================================================
% 【定位】
%   180度回头弯场景下，同一点迹数据，并行运行四种 UKF 后端：
%   jichu / zishiying / imm / imm_3in1。
...
uturn_info = struct();  % uturn ����Ҫ get_turn_info�������Ӻ�����ȡ
omega = pi / 180.0;  % 1��/s

fprintf('========================================================================\n');
fprintf(' ��ͷ��180��: ֱ��(90��) -> ��ת180��Բ��(1��/s) -> ֱ��(270��)\n');
fprintf('========================================================================\n');
fprintf('U  ��ͷ��180�������ƶԱ�MC  N=%d  IMM:CV+CT(Pd-IPDA,Pi=[.90 .10])  U\n', N_MC);
fprintf('========================================================================\n\n');
...
for mc = 1:N_MC
    ...
    fprintf('  �㼣: R1ԭʼ%.0f У׼%.1f | R2ԭʼ%.0f У׼%.1f km\n', ...
    ...
    fprintf('  -> ��������: %s (�ں�RMSE=%.1fkm)\n', s(best_u).name, best_rmses(best_u));

    if ~isnan(s(3).mu_ct_avg_R1(mc))
        fprintf('  IMM CT����: R1 avg=%.0f%% turn=%.0f%% dom=%d | R2 avg=%.0f%% turn=%.0f%% dom=%d\n', ...
...
%% ========================================================================
fprintf('========================================================================\n');
fprintf('          %d �����ؿ���ͳ�ƻ��� (%.0f s)\n', N_MC, elapsed);
fprintf('========================================================================\n');

%% ---- UKF RMSE �Ա� ----
fprintf('\n--- UKF RMSE �����ƶԱ� (km) ---\n');
fprintf('%-18s | %10s | %10s | %10s\n', 'ָ��', 'jichu', 'zishiying', 'imm', 'imm_3in1');
fprintf('%-18s-+%s-+%s-+%s\n', '------------------', '----------', '----------', '----------');
...
print_4way('R2���� RMSE', s(1).rmse_ukf_R2_alg, s(2).rmse_ukf_R2_alg, s(3).rmse_ukf_R2_alg);
...
fprintf('%-18s-+%s-+%s-+%s\n', '------------------', '----------', '----------', '----------');
for m = 1:N_FUS
    print_4way(sprintf('�ں� %s', FUSION_METHODS{m}), ...
        s(1).rmse_fus(:,m), s(2).rmse_fus(:,m), s(3).rmse_fus(:,m));
end
print_4way('�ں�����', s(1).rmse_fus_best, s(2).rmse_fus_best, s(3).rmse_fus_best);

%% ---- �����ʶԱ� ----
fprintf('\n--- �����������ƶԱ� (%%) ---\n');
fprintf('%-18s | %10s | %10s | %10s\n', 'ָ��', 'jichu', 'zishiying', 'imm', 'imm_3in1');
fprintf('%-18s-+%s-+%s-+%s\n', '------------------', '----------', '----------', '----------');
print_4way('UKF���� R1', s(1).imp_ukf_R1, s(2).imp_ukf_R1, s(3).imp_ukf_R1);
print_4way('UKF���� R2', s(1).imp_ukf_R2, s(2).imp_ukf_R2, s(3).imp_ukf_R2);
print_4way('�ں� vs R1', s(1).imp_fus_vs_R1, s(2).imp_fus_vs_R1, s(3).imp_fus_vs_R1);
print_4way('�ں� vs R2', s(1).imp_fus_vs_R2, s(2).imp_fus_vs_R2, s(3).imp_fus_vs_R2);

%% ---- ���� + NIS �Ա� ----
fprintf('\n--- ������������ƶԱ� ---\n');
fprintf('%-18s | %10s | %10s | %10s\n', 'ָ��', 'jichu', 'zishiying', 'imm', 'imm_3in1');
fprintf('%-18s-+%s-+%s-+%s\n', '------------------', '----------', '----------', '----------');
print_4way_pct('������ R1(%%)', s(1).assoc_R1, s(2).assoc_R1, s(3).assoc_R1);
print_4way_pct('������ R2(%%)', s(1).assoc_R2, s(2).assoc_R2, s(3).assoc_R2);
print_4way('NIS��ֵ R1', s(1).nis_mean_R1, s(2).nis_mean_R1, s(3).nis_mean_R1);
print_4way('NIS��ֵ R2', s(1).nis_mean_R2, s(2).nis_mean_R2, s(3).nis_mean_R2);
print_4way_pct('NIS���� R1(%%)', s(1).nis_gate_R1, s(2).nis_gate_R1, s(3).nis_gate_R1);
print_4way_pct('NIS���� R2(%%)', s(1).nis_gate_R2, s(2).nis_gate_R2, s(3).nis_gate_R2);
print_4way('��ʼ֡ R1', s(1).init_fr_R1, s(2).init_fr_R1, s(3).init_fr_R1);
print_4way('��ʼ֡ R2', s(1).init_fr_R2, s(2).init_fr_R2, s(3).init_fr_R2);
...
print_4way('MTL �ں�', s(1).mtl_fus, s(2).mtl_fus, s(3).mtl_fus);
print_4way('���� R1', s(1).brk_R1, s(2).brk_R1, s(3).brk_R1);
print_4way('���� R2', s(1).brk_R2, s(2).brk_R2, s(3).brk_R2);
print_4way('���� �ں�', s(1).brk_fus, s(2).brk_fus, s(3).brk_fus);
...
%% ---- IMM ģ�͸��� ----
fprintf('\n--- IMM ģ�͸��� (�� imm ����) ---\n');
print_imm_mu('CT��ֵ R1(%%)', s(3).mu_ct_avg_R1);
print_imm_mu('CT��ֵ R2(%%)', s(3).mu_ct_avg_R2);
print_imm_mu('CTת�� R1(%%)', s(3).mu_ct_turn_R1);
print_imm_mu('CTת�� R2(%%)', s(3).mu_ct_turn_R2);
print_imm_mu('CTռ��֡ R1', s(3).mu_ct_dom_R1);
print_imm_mu('CTռ��֡ R2', s(3).mu_ct_dom_R2);

%% ---- ����Ա� ----
fprintf('\n--- ����Ա�: ���Ƽ���� (%%) ---\n');
fprintf('%-24s | %10s | %10s\n', 'ָ��', 'zishiying vs jichu.*imm vs jichu.*3in1 vs jichu');
fprintf('%-24s-+%s-+%s\n', '------------------------', '----------', '----------');
...
print_cross_row('Delta �ں�����(%%)', delta_z_vs_j_fus, delta_i_vs_j_fus);
...
for mc_idx = 1:N_MC
    rmses = [s(1).rmse_fus_best(mc_idx), s(2).rmse_fus_best(mc_idx), s(3).rmse_fus_best(mc_idx), s(4).rmse_fus_best(mc_idx)];
