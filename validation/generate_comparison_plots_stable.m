% generate_comparison_plots_stable — 全场景滤波前后对比可视化
% Usage: matlab -batch "addpath(genpath('D:/Desktop/single_target_with-turn')); generate_comparison_plots_stable();"
%
% 策略:
%   1. RMSE数据来自最终验证(final_validate_config51)，2雷达×50seeds权威结果
%   2. 轨迹图仍用2雷达×10seeds平均，确保Config#51真正生效

function generate_comparison_plots_stable()
    ...
    fprintf('============================================================\n');
    fprintf('全场景对比 — RMSE用权威数据，轨迹图用2雷达×%d seeds\n', n_seeds_plot);
    fprintf('============================================================\n\n');

    % --- Load authoritative RMSE from final_validate_config51 ---
    val_result = load(fullfile('D:\Desktop\single_target_with-turn', 'validation', 'final_validate_config51.mat'));
    rmse_default_all = squeeze(val_result.default_rmse);
    rmse_cfg51_all = val_result.scores;
    if size(rmse_default_all,2) == 10, rmse_default_all = rmse_default_all'; end

    fprintf('权威 RMSE 数据已加载（2雷达×50seeds平均）\n\n');
    fprintf('%-22s  %-12s  %-12s  %-8s\n', 'Scenario', 'Default(km)', 'Config#51(km)', 'Delta%');
    fprintf('%-22s  %-12s  %-12s  %-8s\n', repmat('-',1,22),repmat('-',1,12),repmat('-',1,12),repmat('-',1,8));
    for si = 1:n_scenarios
        delta = (rmse_default_all(si) - rmse_cfg51_all(si)) / rmse_default_all(si) * 100;
        fprintf('%-22s  %-12.3f  %-12.3f  %+7.1f%%\n', specs{si}{2}, rmse_default_all(si), rmse_cfg51_all(si), delta);
    end
    fprintf('\n');

    % --- Generate plots: 2 radars × n_seeds_plot ---
    for si = 1:n_scenarios
        ...
        fprintf('[%d/%d] %s...\n', si, n_scenarios, chinese_name);

        prepared = prepare_oracle_tracking_inputs(scenario_name, ...
            struct('random_seed', seed_start, 'truth_turn_rate_deg_per_sec', turn_rate));
        p = prepared.params;

        % Default: 2radar × n_seeds
        rmse_d = 0;
        for r = 1:2
            for sk = 1:n_seeds_plot
                ov = struct('imm_adapt_mode', '3in1', 'random_seed', seed_start+sk-1);
                rmse_d = rmse_d + do_eval(prepared, p, r, seed_start+sk-1, ov);
            end
        end
        rmse_plot_def = rmse_d / (2*n_seeds_plot);

        % Config#51: 2radar × n_seeds
        rmse_c = 0;
        for r = 1:2
            for sk = 1:n_seeds_plot
                ov = struct('imm_adapt_mode', '3in1', 'random_seed', seed_start+sk-1);
                fn = fieldnames(cfg51);
                for fi = 1:numel(fn), ov.(fn{fi}) = cfg51.(fn{fi}); end
                rmse_c = rmse_c + do_eval(prepared, p, r, seed_start+sk-1, ov);
            end
        end
        rmse_plot_cfg = rmse_c / (2*n_seeds_plot);

        plot_single_scene(si, n_scenarios, chinese_name, short_name, ...
            scenario_name, seed_start, turn_rate, cfg51, output_dir, ...
            rmse_plot_def, rmse_plot_cfg);

        fprintf('  Plot RMSE: Default=%.3f, Config#51=%.3f, Delta=%+.1f%%\n', ...
            rmse_plot_def, rmse_plot_cfg, ...
            (rmse_plot_def-rmse_plot_cfg)/max(rmse_plot_def,1e-9)*100);
    end

    % Save CSV using authoritative data
    ...
    for si = 1:n_scenarios
        ...
        fprintf(fid, '%s,%s,%.4f,%.4f,%.2f,%.2f\n', ...
            specs{si}{2}, specs{si}{3}, rmse_default_all(si), rmse_cfg51_all(si), delta, weights(si));
    end
    fclose(fid);
    ...
    grid on; xlim(gca, [0.5, n_scenarios+0.5]);
    title('全场景 RMSE 对比汇总 (权威数据)', 'FontSize', 13, 'FontWeight', 'bold');
...
function rmse_km = do_eval(inp, params, radar_id, seed_val, ov)
    if radar_id == 1
    ...
    for f = 1:numel(snaps)
        ...
        for ti = 1:numel(snaps{f}.trackList)
            ...
            tt = inp.truthTrajs;
            idx = trk.truth_idx;
            if iscell(tt), truth = tt{idx}; else, truth = tt(idx); end
    ...
    clear tpl det snaps;
    if ~isempty(pos_sq), rmse_km = sqrt(mean(pos_sq)) / 1000;
    else, rmse_km = NaN; end
end
...
function plot_single_scene(fig_num, n_total, ch_name, sh_name, ...
    ...
    inp = prepare_oracle_tracking_inputs(scenario_name, ...
        struct('random_seed', seed_val, 'truth_turn_rate_deg_per_sec', turn_rate));
    detList_R1 = inp.detList_R1; truth_input = inp.truth_all;
    truthTrajs = inp.truthTrajs; p1 = inp.params;

    rp = radar_params(p1, 1); rp.imm_adapt_mode = '3in1';
    ...
    rp2 = radar_params(p1, 1); rp2.imm_cv_dwell_time_sec=2500; rp2.imm_ct_dwell_time_sec=660;
    ...
    rp2.imm_transient_ewma_alpha=0.65; rp2.imm_mu_init_CV=0.5; rp2.imm_adapt_mode='3in1';
    ...
    subplot(2,1,2);
    ax2 = gca;
    ax2.XLabel.String = '时间 (s)'; ax2.YLabel.String = '位置误差 (m)'; ax2.FontSize = 10;
    valid = isfinite(trk_def(:,3)) & isfinite(trk_cfg(:,3)) & isfinite(trk_def(:,4));
    t = trk_def(valid, 4);
    if ~isempty(t)
        ed = movmean(trk_def(valid,3), 21); ec = movmean(trk_cfg(valid,3), 21);
    ...
    grid(ax2, 'on');
    txt = sprintf('Default RMSE = %.2f km\nConfig#51 RMSE = %.2f km\nDelta: %+.1f%%', rmse_def, rmse_cfg, (rmse_def-rmse_cfg)/max(rmse_def,1e-9)*100);
...
function pts = collect_track_xy(snaps, t_grid, truthTrajs)
    ...
    for f = 1:numel(snaps)
        ...
        for ti = 1:numel(snaps{f}.trackList)
            ...
            tt = truthTrajs;
            if iscell(tt), truth = tt{trk.truth_idx}; else, truth = tt(trk.truth_idx); end
            ...
            true_lon = interp1(truth.time_sec, truth.lon, t_now, 'linear');
            true_lat = interp1(truth.time_sec, truth.lat, t_now, 'linear');
            if ~all(isfinite([true_lon,true_lat])), continue; end
...
function plot_truth_and_tracks(truthTrajs, detList_R1, def_lons, def_lats, cfg_lons, cfg_lats)
    ax = gca;
    ax.XLabel.String = '经度 (deg E)'; ax.YLabel.String = '纬度 (deg N)'; ax.FontSize = 10;
    ...
    for k = 1:max_det
        if ~isempty(detList_R1{k})
            ...
            if isstruct(dets) && isfield(dets, 'lon')
                det_x = [det_x, dets.lon]; det_y = [det_y, dets.lat];
            end
        end
    end
    if ~isempty(det_x), plot(ax, det_x, det_y, '.', 'Color', [0.9 0.9 0.9], 'MarkerSize', 1); end
    colors = lines(max(numel(truthTrajs), 3));
    for ti = 1:numel(truthTrajs)
        if iscell(truthTrajs), truth = truthTrajs{ti}; else, truth = truthTrajs(ti); end
    ...
    if ~isempty(def_lons)
        plot(ax, def_lons, def_lats, '-s', 'Color', [0.6 0.6 0.6], 'MarkerSize', 3, 'LineWidth', 1.2, 'DisplayName', 'Default Track');
    end
    if ~isempty(cfg_lons)
        plot(ax, cfg_lons, cfg_lats, '-^', 'Color', [0.2 0.6 0.9], 'MarkerSize', 3, 'LineWidth', 1.2, 'DisplayName', 'Config#51 Track');
    end
    legend(ax, 'Location', 'best', 'FontSize', 9);
    axis equal; grid(ax, 'on');
end
